<template>
  <div class="login_container">
    <div class="rating-page">
      ...
      <pd-footer-msg style="margin-top: .2rem"></pd-footer-msg>
    </div>
  </div>
</template>
<script>
...
import { toLogin } from "@/service/pdGetData";
import { Indicator } from "mint-ui";
export default {
  data() {
    return {
      logo: require("@/assets/pd_app_logo.png"),
      selectedTab: 0,
      timer: null,
      count: 59,
      btnText: "发送验证码",
          ...
          .then((res) => {
            ...
            if (res.code === 200) {
              ...
              if (data.isTestAccount === 1) {
                this.$router.push("/panda/confirmLoan");
              } else {
                // 判断用户是否已经进件过，进件过 -> 申请成功界面    未进件  -> 走申请流程
                if (data.isApplyed === 0) {
                  this.$router.push("/panda/loanFlow");
                } else if (data.isApplyed === 1) {
                  this.$router.push("/panda/loanResult");
                }
              }
            }
          })

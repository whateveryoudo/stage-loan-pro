  ...
  <div class="login_container">
    <div class="rating-page">
      ...
      <div class="deal-wrapper">
        ...
        <div>
          我已阅读并同意
          <router-link :to="{ path: '/easy/deal/1' }">《安逸借钱注册协议》</router-link>
          <router-link :to="{ path: '/easy/deal/2' }">《隐私权政策》</router-link>
        </div>
      </div>
      <!--底部下一步按钮组件-->
      <next-btn :enable="validSuc" formType="login" text="登录" @toNext="submitForm"></next-btn>
      <div class="bto-wrapper">
        <router-link to="/easy/regist">
          <span>新用户注册</span>
        </router-link>
      </div>
      <!-- 添加说明 -->
      ...
      <easy-footer-msg style="margin-top: .2rem"></easy-footer-msg>
    </div>
  </div>
</template>
<script>
...
import { toLogin } from "@/service/easyGetData";
import { Indicator } from "mint-ui";
export default {
  data() {
    return {
      logo: require("@/assets/easy_app_logo.png"),
      selectedTab: 0,
      timer: null,
      count: 59,
      btnText: "发送验证码",
          ...
          .then((res) => {
            ...
            if (res.code === 200) {
              ...
              if (data.isTestAccount === 1) {
                this.$router.push("/easy/confirmLoan");
              } else {
                // 判断用户是否已经进件过，进件过 -> 申请成功界面    未进件  -> 走申请流程
                if (data.isApplyed === 0) {
                  this.$router.push("/easy/loanFlow");
                } else if (data.isApplyed === 1) {
                  this.$router.push("/easy/loanResult");
                }
              }
            }
          })

  ...
  <div class="login_container">
    <div class="rating-page">
      ...
      <div class="nav_wrapper" style="display: none">
        <div class="nav_item" @click="toggleTab(0)" :class="{ selected: selectedTab === 0 }">
          账号密码
        </div>
        <span class="divider-line"></span>
        <div class="nav_item" @click="toggleTab(1)" :class="{ selected: selectedTab === 1 }">
          验证码
        </div>
      </div>
      <div class="login_form">
        <section class="field-item-container">
          ...
          <mt-field v-show="selectedTab === 0" label="账号密码" placeholder="请输入账号密码" type="password"
            v-model="userInfo.password"></mt-field>
          <mt-field v-show="selectedTab === 1" label="短信验证码" placeholder="请输入" v-model="userInfo.message"
            :attr="{ maxlength: 4 }">
            <mt-button class="getMsgCode" type="primary" size="normal" @click.native="getMsgCode"
              :class="!isSendBtnEable && 'disabled'" v-text="btnText"></mt-button>
          </mt-field>
        </section>
      </div>
      <div class="deal-wrapper">
        ...
        <div>
          我已阅读并同意
          <router-link :to="{ path: '/emg/deal/1' }">《应急贷款花注册协议》</router-link>
          <router-link :to="{ path: '/emg/deal/2' }">《隐私权政策》</router-link>
        </div>
      </div>
      <!--底部下一步按钮组件-->
      <next-btn :enable="validSuc" formType="login" text="登录" @toNext="submitForm"></next-btn>
      <div class="bto-wrapper">
        <router-link to="/emg/regist">
          <span>新用户注册</span>
        </router-link>
      </div>
      <!-- 添加说明 -->
      <div class="footer" style="margin-top: .4rem">
        ...
        <p>具体利息因个人征信不同略有浮动</p>
        <p>总体贷款年化利率（单利）7.3%-14%</p>
      </div>
      <emg-footer-msg style="margin-top: .2rem"></emg-footer-msg>
    </div>
  </div>
</template>
<script>
...
import { toLogin } from "@/service/emgGetData";
import { Indicator } from "mint-ui";
export default {
  data() {
    return {
      logo: require("@/assets/emg_app_logo.png"),
      selectedTab: 0,
      timer: null,
      count: 59,
      btnText: "发送验证码",
          ...
          .then((res) => {
            ...
            if (res.code === 200) {
              ...
              if (data.isTestAccount === 1) {
                this.$router.push("/emg/confirmLoan");
              } else {
                // 判断用户是否已经进件过，进件过 -> 申请成功界面    未进件  -> 走申请流程
                if (data.isApplyed === 0) {
                  this.$router.push("/emg/loanFlow");
                } else if (data.isApplyed === 1) {
                  this.$router.push("/emg/loanResult");
                }
              }
            }
          })

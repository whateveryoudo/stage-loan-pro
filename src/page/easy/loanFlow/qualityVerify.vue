 ...
 * @LastEditTime: 2022-06-24 00:26:30
-->
...
import { verifyUser } from "@/service/easyGetData";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      const res = await verifyUser({
        name: this.qualityInfo.realName,
        cardNo: this.qualityInfo.idCard,
      });
      Indicator.close();
      if (res && res.code === 200) {
        this.$toast({
        ...
        this.$router.push("/easy/loanFlow/basicInfo");
      } else {
        //表单重置
        this.qualityInfo.realName = "";
        this.qualityInfo.idCard = "";
      }
    },
  },
};

  ...
  methods: {
    ...
    getMsgCode() {
      ...
      if (verifyRules.phone(this.userInfo.username)) {
        getLoginMsg(this.reqId, this.userInfo.username)
          .then((res) => {
            let data = res.data;
            if (data.code == 200) {
              Toast({
    ...
    submitForm() {
      ...
      //校验输入是否正确
      if (
        verifyRules.phone(this.userInfo.username) &&
        verifyRules.email(this.userInfo.mail)
      ) {
        //md5加密密码(提交上传)
        toLogin({ ...this.userInfo, reqId: this.reqId })
          .then((res) => {
            let data = res.data;
            if (data.code == 200) {
              Toast({
